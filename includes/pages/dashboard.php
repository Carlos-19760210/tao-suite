...
function tao_crm_page_dashboard() {
    ...
    // ── Período ────────────────────────────────────────────────────────────────
    $dias  = max( 7, min( 180, intval( $_GET['dias'] ?? 30 ) ) );
    $desde = gmdate( 'c', strtotime( "-{$dias} days" ) );

    // ── Buscar cards ──────────────────────────────────────────────────────────
    $rc_all = tao_crm_api(
        "/crm_cards?workspace_id=eq.$ws_id" .
        "&select=id,fechado,estagio_id,responsavel_id,criado_em,atendimento_humano,valor_oportunidade" .
        "&limit=2000"
    );
    ...
    // Total em oportunidades (cards abertos)
    ...
    <div class="wrap tao-crm-wrap">
        ...
        <div class="crm-topbar-dash">
            <h1>&#x1F4CA; Dashboard &mdash; <?php echo esc_html( $ws['nome'] ); ?></h1>
            <form method="get" class="crm-period-form">
                ...
                <select name="dias" onchange="this.form.submit()">
                    <?php foreach ( [ 7 => '7 dias', 30 => '30 dias', 90 => '90 dias', 180 => '6 meses' ] as $v => $l ) : ?>
        ...
        <div class="crm-dash-kpi-row">
            ...
            <div class="crm-dash-kpi-card kpi-amber">
                <span class="kpi-label">Total em oportunidades</span>
                <span class="kpi-value">R$&nbsp;<?php echo number_format( $total_oportunidades, 0, ',', '.' ); ?></span>
                <span class="kpi-sub">soma dos cards abertos</span>
            </div>
