...
function cbpm_page_campanhas() {
    ...
    <?php else: // new / edit ?>
        ...
        <form method="post" enctype="multipart/form-data" class="cbpm-form cbpm-fv">
            ...
            <div class="cbpm-fg">
                <label>Negócio *</label>
                <select name="cliente_id"<?php if ( $cbpm_locked ) echo " disabled"; ?> required>
...
function cbpm_campanha_view( $campanha, $is_admin, $status_labels, $status_colors, $listas_salvas = [] ) {
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:16px;align-items:start;margin-bottom:16px">
        <?php if ( $tem_grafico ): ?>
        <div style="background:#fff;border:1px solid #dcdcde;border-radius:4px;padding:20px;min-width:0">
            <h3 style="margin:0 0 12px;font-size:13px;text-transform:uppercase;letter-spacing:.4px;color:#50575e">Envios por hora</h3>
            <div style="position:relative;height:200px"><canvas id="cbpm_chart_bar"></canvas></div>
        </div>
        <?php endif; ?>
        <div style="background:#fff;border:1px solid #dcdcde;border-radius:4px;padding:20px;min-width:0">
            <h3 style="margin:0 0 12px;font-size:13px;text-transform:uppercase;letter-spacing:.4px;color:#50575e">Distribuição</h3>
            <div style="position:relative;height:220px"><canvas id="cbpm_chart_pie"></canvas></div>
        </div>
    </div>
    <?php endif; ?>
